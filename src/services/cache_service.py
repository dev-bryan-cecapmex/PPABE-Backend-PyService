...
class JobCache:
    """
    Cache aislado por ejecución (endpoint/run).
    - catalogs: snapshot INMUTABLE (no se afecta por refresh global)
    - local: dict libre para lo que quieras cachear dentro del proceso (maps, lookups, etc.)
    """
    job_id: str
    created_at: float
    catalogs: Dict[str, Dict[str, Any]]  # snapshot inmutable (MappingProxyType por nivel)
    local: Dict[str, Any]               # cache local mutable solo de este job


class CacheService:
    """
    Cache global de catálogos + cache por job (por ejecución).
    ✅ No hay cache de beneficiarios aquí.
    ✅ Un refresh global NO rompe jobs en curso (refresh atómico + snapshots).
    """

    def __init__(self, ttl_seconds: int = 3600, job_ttl_seconds: int = 7200):
        self.ttl_seconds = ttl_seconds
        self.job_ttl_seconds = job_ttl_seconds

        self._lock = threading.RLock()

        # Cache global de catálogos (mutable internamente, pero se expone como snapshot inmutable)
        self._catalogs_cache: Dict[str, Dict[str, Any]] = {}
        self._catalogs_loaded_at: Optional[float] = None

        # Jobs activos -> JobCache
        self._jobs: Dict[str, JobCache] = {}
    ...
    def _index_catalog(self, rows):
        """
        Construye índices por id / nombre para búsquedas rápidas.
        """
        by_id: Dict[str, Dict[str, Any]] = {}
        by_name: Dict[str, Dict[str, Any]] = {}
    ...
    # -------------------------
    # Catálogos globales (refresh atómico)
    # -------------------------
    def refresh_catalogs_cache(self, catalog_types):
        """
        Refresca catálogos globales de forma ATÓMICA (swap).
        Nunca hace clear() para no romper lecturas concurrentes.
        """
        with self._lock:
            start = time.perf_counter()
            Logger.add_to_log("info", "🔄 Refrescando cache global de catálogos...")
            ...
            elapsed = time.perf_counter() - start
            Logger.add_to_log("info", f"✅ Cache global de catálogos refrescado en {elapsed:.2f}s")
            Logger.add_to_log("info", f"   📊 Catálogos cargados: {len(self._catalogs_cache)} tipos")

    def get_catalogs_snapshot(self) -> Dict[str, Dict[str, Any]]:
        """
        Snapshot INMUTABLE del cache global.
        - Si está expirado y ya hay catálogos, refresca (si tú decides llamarlo).
        - OJO: Aquí NO adivinamos tipos si nunca se cargaron.
        """
        with self._lock:
            # No forzamos refresh si no sabemos qué tipos cargar
            # (mejor que el caller pase catalog_types en su arranque)
            snapshot = {
                k: MappingProxyType(v) for k, v in self._catalogs_cache.items()
            }
    ...
    # -------------------------
    def create_job_cache(self) -> JobCache:
        """
        Crea un cache por ejecución con snapshot de catálogos globales.
        """
        with self._lock:
            self._cleanup_jobs_locked()

            job_id = str(uuid.uuid4())
            catalogs_snapshot = self.get_catalogs_snapshot()

            job = JobCache(
                job_id=job_id,
                created_at=self._now(),
                catalogs=catalogs_snapshot,  # snapshot inmutable
                local={},                    # cache local mutable del job
            )
            self._jobs[job_id] = job
            return job

    def get_job_cache(self, job_id: str) -> Optional[JobCache]:
        with self._lock:
            self._cleanup_jobs_locked()
            return self._jobs.get(job_id)

    def delete_job_cache(self, job_id: str) -> None:
        with self._lock:
            self._jobs.pop(job_id, None)

    def _cleanup_jobs_locked(self) -> None:
        """
        Limpia jobs viejos para no crecer infinito.
        """
        now = self._now()
        to_delete = []
        for job_id, job in self._jobs.items():
            if (now - job.created_at) > self.job_ttl_seconds:
                to_delete.append(job_id)
        for job_id in to_delete:
            self._jobs.pop(job_id, None)

    # -------------------------
    # Búsquedas contra snapshot (rápidas y seguras)
    # -------------------------
    def find_in_job_catalog_by_id(self, job: JobCache, catalog_type: str, _id: str) -> Optional[Dict[str, Any]]:
        cat = job.catalogs.get(catalog_type)
        if not cat:
            return None
        return cat.get("by_id", {}).get(str(_id))

    def find_in_job_catalog_by_name(self, job: JobCache, catalog_type: str, name: str) -> Optional[Dict[str, Any]]:
        cat = job.catalogs.get(catalog_type)
        if not cat:
            return None
        return cat.get("by_name", {}).get(self._norm_name(name))

    # -------------------------
    # Endpoints admin (seguros)
    # -------------------------
    def force_refresh_global_catalogs(self, catalog_types):
        """
...
# Singleton
cache_service = CacheService(ttl_seconds=3600, job_ttl_seconds=7200)
