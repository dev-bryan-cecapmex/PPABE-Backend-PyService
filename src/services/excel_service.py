        ...
        try:
            ...
            for row in rows:
                ...
                if not id_beneficiario:
                    Logger.add_to_log("info","Se dara de alta el usario")
                    id_new_beneficiario = str(uuid.uuid4())
                    Logger.add_to_log("info", f"No existe - Nuevo UUID: { id_new_beneficiario }")
                    Logger.add_to_log("info", row)
                                
                    # Inserta beneficiario 
                    #BeneficiariosService.add_beneficiario(id_new_beneficiario, beneficiario_map)
                    new_beneficiarios.append({
                        'id': id_new_beneficiario,
                        **{k:row[k] for k in group_one_df}
                    })
                else:   
            ...
            if new_beneficiarios:
                Logger.add_to_log("info", f"Estos son los beneficiarios que se darna de alta {new_beneficiarios}")
